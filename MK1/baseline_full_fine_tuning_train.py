...
def main():
    ...
    for dataset_name in dataset_map:
        ...
        data = datasets.DatasetDict({
            'train': datasets.Dataset.from_dict({ k: dataset_map[dataset_name]['data']['train'][:1000][dataset_map[dataset_name]['attributes'][k]] for k in ['text', 'summary'] }),
            'test': datasets.Dataset.from_dict({ k: dataset_map[dataset_name]['data']['train'][:10][dataset_map[dataset_name]['attributes'][k]] for k in ['text', 'summary'] }),
        })
        tokens = data.map(
        ...
        training_args = transformers.TrainingArguments(
            output_dir=f'./full-fine-tuning-{dataset_name}-t5-base',
            eval_strategy='no',
            learning_rate=5e-5,
        ...
        with open(f'model-t5-base-full-fine-tuning-{dataset_name}.pickle', 'wb') as file:
