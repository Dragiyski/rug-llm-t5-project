...
def main():
    ...
    with open('model-t5-base-full-fine-tuning-cnn.pickle', 'rb') as file:
        model_cnn = pickle.load(file).to(model_run_device)
    with open('model-t5-base-full-fine-tuning-samsun.pickle', 'rb') as file:
        model_samsun = pickle.load(file).to(model_run_device)
    with open('model-t5-base-full-fine-tuning-mixed.pickle', 'rb') as file:
        model_mixed = pickle.load(file).to(model_run_device)
    ...
    for dataset_name in dataset_map:
        ...
        for example in tqdm(tokens['test'], desc='Generating summaries'):
            ...
            mixed_out_ids = model_mixed.generate(input_ids=input_ids, max_length=192, min_length=10, num_beams=4, length_penalty=2.0, use_cache=False).to(output_device)
            ...
            mixed_summary = tokenizer.decode(mixed_out_ids[0], skip_special_tokens=True)
            ...
            predictions['mixed'].append(mixed_summary)

        with open(f'baseline-full-fine-tuning-predictions-{dataset_name}.pickle', 'wb') as file:
