import sys
import transformers, datasets, torch, evaluate, functools, pickle, multiprocessing, peft
...
def main():
    ...
    def perform_trainig(train_data, test_data, *, label, r):
        ...
        lora_model = peft.get_peft_model(model, config).to(model_run_device)

        training_args = transformers.TrainingArguments(
            output_dir=f'./lora-{dataset_name}-t5-base',
            eval_strategy='no',
            learning_rate=5e-5,
        ...
        trainer.train()
        with open(f'model-t5-base-lora-fine-tuning-{label}-{r}.pickle', 'wb') as file:
            pickle.dump(trainer.model.to(output_device), file)

    for r in range(1, 6):
        for dataset_name in dataset_map:
            perform_trainig(
                datasets.Dataset.from_dict({ k: dataset_map[dataset_name]['data']['train'][:1000][dataset_map[dataset_name]['attributes'][k]] for k in ['text', 'summary'] }),
                datasets.Dataset.from_dict({ k: dataset_map[dataset_name]['data']['test'][:10][dataset_map[dataset_name]['attributes'][k]] for k in ['text', 'summary'] }),
                r=r,
                label=dataset_name
            )
        perform_trainig(
            datasets.Dataset.from_dict({ k: dataset_map['cnn']['data']['train'][1000:1500][dataset_map['cnn']['attributes'][k]] + dataset_map['samsun']['data']['train'][500:1000][dataset_map['samsun']['attributes'][k]] for k in ['text', 'summary'] }),
            datasets.Dataset.from_dict({ k: dataset_map['cnn']['data']['test'][100:150][dataset_map['cnn']['attributes'][k]] + dataset_map['samsun']['data']['test'][50:100][dataset_map['samsun']['attributes'][k]] for k in ['text', 'summary'] }),
            r=r,
            label='mixed'
        )
